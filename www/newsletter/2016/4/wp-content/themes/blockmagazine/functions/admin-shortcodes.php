...
function tmnf_enqueue_shortcode_js () {
	...
	if ( is_admin() ) {} else {
		wp_enqueue_script( 'tmnf-shortcodes', get_template_directory_uri() . '/functions/js/shortcodes.js', array( 'jquery', 'jquery-ui-tabs' ), true );
	} // End IF Statement
...
function tmnf_magmenu($atts, $content = null) {
	...
	if(!empty($category)){
		$query .= 'meta_key=post_views_count&orderby=meta_value_num&showposts=4&order=DESC&category_name='.$category;
	}
	if(!empty($query)){
		$query .= $query;
	}
	...
	<?php while ($wp_query->have_posts()) : $wp_query->the_post(); ?>
		<li><?php get_template_part('/includes/post-types/menu-post'); ?></li>
	<?php endwhile; ?>
	</ul>
    <?php wp_reset_query(); ?>
...
function tmnf_magmenu2($atts, $content = null) {
	...
	$wp_query = new WP_Query();
	if(!empty($category)){
		$query .= 'meta_key=post_views_count&orderby=meta_value_num&showposts=5&order=DESC&category_name='.$category;
	}
	if(!empty($query)){
		$query .= $query;
	}
