...
function get_comment_author_link( $comment_ID = 0 ) {
	...
	if ( empty( $url ) || 'http://' == $url )
		...
		$return = "<span style='color:#333;'>$author</span> <span style='color:#333;'> (수정)</span>";
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$fields   =  array(
		...
		'url'    => '<p class="comment-form-url"><label for="url">' . 부서명. ( $req ? ' <span class="required">*</span>' : '' ). '</label> ' .
		           	'<input id="url" name="url" ' . ( $html5 ? 'type="url"' : 'type="text"' ) . ' value="" size="30" required="required"/></p>',
	);
