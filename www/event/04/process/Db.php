  ...
  class Db{
     ...
     */
     public function insert($name,$company,$phone,$customerNum,$salesManager,$password,$privacy_agreement,$comment){
       ...
       try {
         ...
         $stmt = $dbh->prepare("INSERT INTO event_participants_04 (name,company,phone,customerNum,salesManager,password,privacy_agreement,comment) VALUES (:name,:company,:phone,:customerNum,:salesManager,:password,:privacy_agreement,:comment)");
    ...
    public function update($name,$company,$phone,$customerNum,$salesManager,$comment,$id) {
      $result = false;
		  ...
		  try {
        $dbh = new PDO("mysql:host=$this->DbHost;dbname=$this->DbName",$this->DbUser,$this->DbPass, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8"));
			  $stmt = $dbh->prepare("UPDATE event_participants_04 SET name = :name, company = :company, phone = :phone, customerNum = :customerNum, salesManager = :salesManager, comment = :comment WHERE id = :id");
    ...
    public function select_pw($id){
      ...
      try {
        $dbh = new PDO("mysql:host=$this->DbHost;dbname=$this->DbName",$this->DbUser,$this->DbPass, array(PDO::MYSQL_ATTR_INIT_COMMAND=>"SET NAMES utf8"));
        $stmt = $dbh->prepare("SELECT * FROM event_participants_04 WHERE id=:id");
        $stmt->bindParam(':id',$id);
        $stmt->execute();
